...
// txOutboxStore is a transaction-scoped OutboxStore.
...
// PollReady returns up to limit unpublished rows eligible for delivery.
...
// MarkFailed increments attempts, records last_error, and advances next_attempt_at.
...
func pollReadyOutboxEvents(ctx context.Context, q querier, limit int) ([]*domain.OutboxEvent, error) {
	...
	const query = `
SELECT id, aggregate_type, aggregate_id, event_id, channel, payload,
...
func markOutboxFailed(ctx context.Context, q querier, id uuid.UUID, lastErr string) error {
	...
	// We read the current attempts from the DB so the backoff is consistent
	// even across poller restarts.
	const query = `
UPDATE event_outbox
...
func scanOutboxEvent(row rowScanner) (*domain.OutboxEvent, error) {
	...
	err := row.Scan(
		...
		&e.Attempts, &e.LastError, &e.NextAttemptAt,
	)
	if err != nil {
