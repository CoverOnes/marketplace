...
// Fixed identity values shared across the table. Only the secret, account type
// and timestamp vary per case, so those are the helper's parameters. The signed
// tier is always the honest fixRealTier; forged-tier tests override the wire
// header after construction so the wire value diverges from the signed value.
const (
...
func TestVerifyGatewaySignature(t *testing.T) {
	...
	t.Run("valid POST with body signature passes", func(t *testing.T) {
		...
		req := newSignedRequest(t, testSecret, fixAccountType, now, http.MethodPost, "/v1/listings", body)
	...
	t.Run("body replay: same body on different path is rejected", func(t *testing.T) {
		// A signature over POST /v1/listings must NOT verify on POST /v1/bids,
		// because the path is included in the canonical string.
		body := []byte(`{"amount":100}`)
		reqA := newSignedRequest(t, testSecret, fixAccountType, now, http.MethodPost, "/v1/listings", body)
		// Grab the signature computed for /v1/listings and apply it to /v1/bids.
		sig := reqA.Header.Get(headerGatewaySignature)

		reqB := newSignedRequest(t, testSecret, fixAccountType, now, http.MethodPost, "/v1/bids", body)
